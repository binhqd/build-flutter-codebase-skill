# Workflow: Build Codebase

Scaffold a complete Flutter project from zero to production-ready.
...
1. Look for `.build-tracker.md` in project root.
   ...
   - **Not found**: Read [tracker-template.md](tracker-template.md) → create `.build-tracker.md` in project root.
...
Read [tech-stack-dictionary.md](tech-stack-dictionary.md). Present the two recommended approaches (BLoC vs Riverpod) with trade-offs. Let the user pick.
...
For EVERY package from the dictionary's key packages table, verify before adopting. Follow the "Dependency Security & Compatibility Verification" section in [tech-stack-dictionary.md](tech-stack-dictionary.md):
...
4. Key DO/DON'T rules from [implementation-guidelines.md](implementation-guidelines.md)
...
Apply strict lint rules. See [implementation-guidelines.md](implementation-guidelines.md) for the exact configuration.

**Step 2.3 — Set up flavors (dev / staging / prod) [LOW]**

See [build-deploy.md](build-deploy.md). Configure:
...
Adapt generated structure to match the layered architecture from [architecture-layers.md](architecture-layers.md). Create the full directory tree (core/, features/, shared/).
...
1. Read [implementation-guidelines.md](implementation-guidelines.md) for Dart/Flutter rules.
...
- Deep linking config
- See [navigation-routing.md](navigation-routing.md)
...
**Before writing code**, read [authentication.md](authentication.md).
...
Target: **>90% coverage** on business logic. Read [testing-strategy.md](testing-strategy.md).
...
Read [build-deploy.md](build-deploy.md).
